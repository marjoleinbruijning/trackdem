...
if debug:
	print 'Calling "which" with command: "%s"' % 'libavpath'
avconv = which(libavpath)
if debug:
	print 'Calling "which" with command: "%s"' % 'exiftool'
...
def getTargetDirNamePrefix(fileName):
	if debug:
		print 'Entering getTargetDirNamePrefix with fileName = "%s"' % fileName
	if exiftool:
		try:
			...
			if debug:
				print 'Got targetDirNamePrefixRaw = "%s"' % targetDirNamePrefixRaw
